...
<body class="bg-slate-50 antialiased min-h-screen flex">
    ...
    <!-- Sidebar -->
    <aside id="sidebar" class="w-64 bg-slate-900 text-white flex flex-col justify-between shrink-0 border-r border-slate-800 fixed inset-y-0 left-0 z-40 -translate-x-full md:translate-x-0 transition-transform duration-300">
            ...
            <nav class="flex-1 px-3 py-4 space-y-0.5">
                ...
                <a href="{{ route('admin.reviews') }}"
                   class="flex items-center px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-150 gap-3 {{ request()->routeIs('admin.reviews*') ? 'bg-orange-600 text-white shadow-md shadow-orange-900/40' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    ...
                    @if($ulasanRendah > 0)
                        <span class="ml-auto text-[10px] font-black bg-amber-500 text-white rounded-full px-2 py-0.5">{{ $ulasanRendah }}</span>
                    @endif
                </a>
                @endcan
                ...
                <a href="{{ route('admin.returns') }}"
                   class="flex items-center px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-150 gap-3 {{ request()->routeIs('admin.returns*') ? 'bg-orange-600 text-white shadow-md shadow-orange-900/40' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    ...
                    @if($returPending > 0)
                        <span class="ml-auto text-[10px] font-black bg-red-500 text-white rounded-full px-2 py-0.5">{{ $returPending }}</span>
                    @endif
                </a>
                @endcan
                ...
                <a href="{{ route('admin.rpay.withdrawals') }}"
                   class="flex items-center px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-150 gap-3 {{ request()->routeIs('admin.rpay.withdrawals*') ? 'bg-orange-600 text-white shadow-md shadow-orange-900/40' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    ...
                    @if($cairPending > 0)
                        <span class="ml-auto text-[10px] font-black bg-red-500 text-white rounded-full px-2 py-0.5">{{ $cairPending }}</span>
                    @endif
                </a>
                @endcan
                @endcanany
            ...
            <div class="p-3 border-t border-slate-800 bg-slate-950/60 shrink-0">
                <div class="flex items-center justify-between px-2 py-2">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-full bg-gradient-to-br from-orange-500 to-rose-500 flex items-center justify-center font-bold text-white shadow-md text-sm">
                    ...
                    <form action="{{ route('admin.logout') }}" method="POST">
                        @csrf
                        <button type="submit" class="text-slate-500 hover:text-red-400 transition p-1.5 rounded-lg hover:bg-slate-800" title="Keluar">
        ...
        <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 sticky top-0 z-30 shadow-sm">
            ...
            <div class="flex items-center gap-3">
                <div class="hidden sm:flex items-center gap-2 text-sm text-gray-500">
                    <div class="w-7 h-7 rounded-full bg-gradient-to-br from-orange-500 to-rose-500 flex items-center justify-center font-bold text-white text-xs">
                        {{ strtoupper(substr(Auth::user()->name ?? 'A', 0, 1)) }}
                    </div>
